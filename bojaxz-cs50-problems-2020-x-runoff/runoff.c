...
{
    // TODO
    // Tally votes for non elminated candidates
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
            non_eliminated_count = candidates[i].votes;
            total_count++;
        }
    }
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated && candidates[i].votes == non_eliminated_count)
        {
            compare++;
        }
    }
    return compare == total_count;
}
